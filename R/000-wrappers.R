...
#' @useDynLib rravif, .registration = TRUE
#' @keywords internal
NULL
...
#' @export
`$<-.savvy_rravif__sealed` <- function(x, name, value) {
  class <- gsub("__bundle$", "", class(x)[1])
  stop(class, " cannot be modified", call. = FALSE)
}

#' @export
`[[<-.savvy_rravif__sealed` <- function(x, i, value) {
  class <- gsub("__bundle$", "", class(x)[1])
  stop(class, " cannot be modified", call. = FALSE)
}
...
#' @noRd
`encode_avif` <- function(`nr`, `height`, `width`, `quality`, `speed`) {
